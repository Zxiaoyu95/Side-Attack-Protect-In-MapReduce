...
public class MRRB_dAge {
	static int numReduceTasks =7;
	static ArrayList<String> dAge = new ArrayList<String>();
	static ArrayList<String> S_dAge = new ArrayList<String>();
/*job1*/
	 public static class MyMapper extends Mapper<LongWritable,Text,Text,Text>{
		@Override
		protected void setup(Mapper<LongWritable, Text, Text, Text>.Context context)
				throws IOException, InterruptedException {
			dAge.add("7");dAge.add("6");dAge.add("5");dAge.add("4");
			dAge.add("3");dAge.add("2");dAge.add("1");dAge.add("0");
			super.setup(context);
		}
		@Override
		protected void map(LongWritable key, Text value, Context context)
			...
			context.write(new Text(values[1].replace("\"", "")), new Text("1"));
	...
	public static class ShuffleReduce extends Reducer<Text,Text,Text,Text>{
		...
		protected void cleanup(Reducer<Text, Text, Text, Text>.Context context)
				throws IOException, InterruptedException {
			for (String entry : dAge){
				if(! S_dAge.contains(entry)){
	...
	static class MyCombiner extends Reducer<Text,Text,Text,Text>{
		...
		protected void reduce(Text key, Iterable<Text> values,Context context) throws IOException, InterruptedException {
			...
			if(! S_dAge.contains(key.toString())){
				S_dAge.add(key.toString());
			}
			context.write(key, new Text(String.valueOf(count)));
		}
	}
...
//    	Job job1 =Job.getInstance(conf,"job1");
    	...
    	//设置job的运行主类
    	job1.setJarByClass(MRRB_dAge.class);
...
//        Job job2 =Job.getInstance(conf,"job2");
        Job job2 =new Job();
    	//设置job的运行主类
        job2.setJarByClass(MRRB_dAge.class);
