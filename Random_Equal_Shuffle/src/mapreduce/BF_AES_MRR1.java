...
public class BF_AES_MRR1 {
	...
	static BloomFilter bloomFilter1 = new BloomFilter(10000,6,Hash.MURMUR_HASH);
	static BloomFilter bloomFilter2 = new BloomFilter(10000,6,Hash.MURMUR_HASH);
//	static BloomFilter bloomFilter3= new BloomFilter(10000,6,Hash.MURMUR_HASH);
/*job1*/
...
//			context.write(new Text(new String(JAES.parseByte2HexStr(encryptK))), new Text(new String(JAES.parseByte2HexStr(encryptV))));
			bloomFilter1.add(new Key(values[5].getBytes()));
	...
	static class MyCombiner extends Reducer<Text,Text,Text,Text>{
		...
		protected void reduce(Text key, Iterable<Text> values,Context context) throws IOException, InterruptedException {
			...
			byte[] encryptV=JAES.encrypt(String.valueOf(count), password);
			if(bloomFilter1.membershipTest(new Key(key.toString().getBytes()))){
				context.write(key, new Text(new String(JAES.parseByte2HexStr(encryptV))));
			}
			else{
...
/*job2*/
	 public static class MyMapper2 extends Mapper<LongWritable,Text,Text,Text>{
			...
			protected void map(LongWritable key, Text value, Context context)
				...
				bloomFilter2.add(new Key("1".getBytes()));
			}
			@Override
			protected void cleanup(Mapper<LongWritable, Text, Text, Text>.Context context)
			throws IOException, InterruptedException {
		...
		public static class ShuffleReduce2 extends Reducer<Text,Text,Text,Text>{
			...
			protected void reduce(Text key, Iterable<Text>values,Context context) throws IOException, InterruptedException {
				...
				if(bloomFilter1.membershipTest(new Key(key.toString().getBytes())) && count!=0){
...
//    	Job job1 =Job.getInstance(conf,"job1"); 
    	Job job1 =new Job();
    	//设置job的运行主类
    	job1.setJarByClass(BF_AES_MRR1.class);
...
//        Job job2 =Job.getInstance(conf,"job2");
        Job job2 =new Job();
    	//设置job的运行主类
        job2.setJarByClass(BF_AES_MRR1.class);
